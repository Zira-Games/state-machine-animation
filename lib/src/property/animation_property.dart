...
class AnimationProperty<T extends dynamic, S> extends Equatable {
  ...
  final _stateValueCache = ExpireCache<AnimationState, T>(expireDuration: const Duration(milliseconds: 10000), sizeLimit: 100);
  ...
  Serializer get serializer => _serializer ?? (AnimationPropertyState state) => name!.split(".").reversed.fold<dynamic>(state.value, (previousValue, key) => { key: previousValue });
  final bool shouldCache;

  final AnimationContainer<S, AnimationModel>? container;

  AnimationProperty({ required this.initialValue, this.name, this.container, this.stateEvaluator, this.keyEvaluator, this.tween, this.interpolator, this.defaultCurve, CurveEvaluator<S>? evaluateCurve, Serializer? serializer, this.evaluateKeyframes, this.shouldCache = false})
  ...
  @override List<Object?> get props => [initialValue, name, stateEvaluator, keyEvaluator, tween, interpolator, defaultCurve, _evaluateCurve, evaluateKeyframes, serializer, shouldCache];

  PropertyAnimation<T> getAnimation(Stream<AnimationStateMachineValue<S>?> sourceStream){
    final behaviorSubject = BehaviorSubject<AnimationPropertyState<T>>.seeded(
      AnimationPropertyState.ofMoment(AnimationPropertyMoment(initialValue, 0, null)),
    );
    final propertyStateStream = sourceStream.distinct().map((machineState) => behaviorSubject.value.updateWith(_getMoment(machineState)));
    behaviorSubject.addStream(propertyStateStream);
    return behaviorSubject;
  }

  AnimationPropertyMoment<T> _getMoment(AnimationStateMachineValue<S>? machineState) => AnimationPropertyMoment(
    machineState == null ? initialValue : getValue(machineState.state, machineState.sourceState, machineState.config),
    machineState?.state.duration(machineState.sourceState, machineState.config) ?? 0,
    machineState?.state
  );

  T getValue(AnimationState state, S sourceState, AnimationStateMachineConfig config){
  ...
  PropertyTimeline<AnimationProperty<T, S>, T, S> getTimeline(AnimationContainer? container, Transition transition, S sourceState){
    if( evaluateKeyframes != null ){
      final forwardKeyframes = evaluateKeyframes!(transition, sourceState);
      if( forwardKeyframes != null){
        return PropertyTimeline<AnimationProperty<T, S>, T, S>(container, this, transition.copyWith(defaultKeyframes: forwardKeyframes));
      }
...
class IntegerAnimationProperty<S> extends AnimationProperty<int, S> {
  IntegerAnimationProperty({ super.name, super.initialValue = 0, super.stateEvaluator, super.keyEvaluator, super.interpolator, super.defaultCurve, super.evaluateCurve, super.serializer, super.evaluateKeyframes }) :
        super(tween: Tween<int>());
}

class DoubleAnimationProperty<S> extends AnimationProperty<double, S> {
  DoubleAnimationProperty({ super.name, super.initialValue = 0.0, super.stateEvaluator, super.keyEvaluator, super.interpolator, super.defaultCurve, super.evaluateCurve, super.serializer, super.evaluateKeyframes }) :
        super(tween: Tween<double>());
}

class ModdedDoubleAnimationProperty<S> extends AnimationProperty<double, S> {
  ModdedDoubleAnimationProperty({ required double mod, super.name, super.initialValue = 0.0, super.stateEvaluator, super.keyEvaluator, super.interpolator, super.defaultCurve, super.evaluateCurve, super.serializer, super.evaluateKeyframes }) :
        super(tween: ModdedDoubleTween(mod));
}

class SizeAnimationProperty<S> extends AnimationProperty<OffsetBase?, S> {
  SizeAnimationProperty({ super.name, super.initialValue = const Size(0.0, 0.0), super.stateEvaluator, super.keyEvaluator, super.interpolator, super.defaultCurve, super.evaluateCurve, super.serializer, super.evaluateKeyframes }) :
        super(tween: SizeTween());
}

class ColorAnimationProperty<S> extends AnimationProperty<Color?, S> {
  ColorAnimationProperty({ super.name, super.initialValue = const Color(0x00000000), super.stateEvaluator, super.keyEvaluator, super.interpolator, super.defaultCurve, super.evaluateCurve, super.serializer, super.evaluateKeyframes }) :
        super(tween: ColorTween());
}

class BoolAnimationProperty<S> extends AnimationProperty<bool, S> {
  BoolAnimationProperty({ super.name, super.initialValue = false, super.stateEvaluator, super.keyEvaluator, super.defaultCurve, super.evaluateCurve, super.serializer, super.evaluateKeyframes }) :
        super(interpolator: (bool from, bool to, double progress) => progress >= 0.5 ? from : to );
}

class StringAnimationProperty<S> extends AnimationProperty<String, S> {
  StringAnimationProperty({ super.name, super.initialValue = "", super.stateEvaluator, super.keyEvaluator, super.tween, super.defaultCurve, super.evaluateCurve, super.serializer, super.evaluateKeyframes }) :
...
String stringInterpolator(String? from, String? to, double progress){
  ...
  final rest = startsWith ? (to ?? "").substring((from ?? "").length) : (to ?? "");
  final addedCharacterLength = (rest.length * progress).ceil();
  return "$from${rest.substring(0, addedCharacterLength)}";
}

int directionOf<T>(T previous, T current) => isSubtype<T, Comparable>() ? Comparable.compare(previous as Comparable, current as Comparable) : 0;
bool isSubtype<S, T>() => <S>[] is List<T>;

class ModdedDoubleTween extends Tween<double> {
  final double mod;

  ModdedDoubleTween(this.mod, {
    super.begin,
    super.end,
  });
